...
class MainActivity : AppCompatActivity() {
    ...
    /**
     * Consulta si hay una version mas nueva registrada en el servidor y, de
     * haberla, muestra un aviso con boton para descargarla. Si el admin la
     * marco como obligatoria, el aviso no se puede cerrar sin actualizar.
     */
    private fun revisarVersionNueva() {
        lifecycleScope.launch {
            try {
                val respuesta = WebApiClient.get().obtenerVersion()
                if (!respuesta.hay_version || respuesta.version_name.isNullOrBlank()) return@launch

                val disponible = respuesta.version_name
                val instalada = BuildConfig.VERSION_NAME

                if (!esVersionMasNueva(disponible, instalada)) return@launch

                val prefs = getSharedPreferences("datgar_version", Context.MODE_PRIVATE)
                val yaAvisada = prefs.getString("version_avisada", "")

                if (!respuesta.obligatorio && yaAvisada == disponible) return@launch
                ...
                val dialogo = AlertDialog.Builder(this@MainActivity)
                    ...
                    .setMessage(
                        ...
                        (respuesta.notas?.takeIf { it.isNotBlank() } ?: "")
                    )
                    .setCancelable(!respuesta.obligatorio)
                    .setPositiveButton("Actualizar") { _, _ ->
                        try {
                            startActivity(
                                android.content.Intent(
                                    android.content.Intent.ACTION_VIEW,
                                    android.net.Uri.parse(respuesta.apk_url)
                                )
                            )
                        } catch (e: Exception) {
                            Toast.makeText(this@MainActivity, "No se pudo abrir el enlace de descarga.", Toast.LENGTH_SHORT).show()
                        }
                        if (respuesta.obligatorio) finish()
                    }

                if (!respuesta.obligatorio) {
                    dialogo.setNegativeButton("Más tarde") { _, _ ->
                        prefs.edit().putString("version_avisada", disponible).apply()
                    }
                }
    ...
    private fun esVersionMasNueva(disponible: String, instalada: String): Boolean {
        ...
        for (i in 0 until maxLen) {
            val a = partesDisponible.getOrElse(i) { 0 }
            val b = partesInstalada.getOrElse(i) { 0 }
            if (a != b) return a > b
        }
        return false
    }
    ...
    private fun configurarMenuLateral() {
        ...
        // Icono de búsqueda del header → abre el drawer y enfoca el campo
        binding.btnBuscar.setOnClickListener {
            binding.drawerLayout.openDrawer(androidx.core.view.GravityCompat.START)
            binding.etBuscar.requestFocus()
            binding.etBuscar.post {
                val imm = getSystemService(Context.INPUT_METHOD_SERVICE) as android.view.inputmethod.InputMethodManager
                imm.showSoftInput(binding.etBuscar, android.view.inputmethod.InputMethodManager.SHOW_IMPLICIT)
            }
        }

        // Bottom navigation
        binding.navInicio.setOnClickListener {
            // Ya estamos en inicio; solo resetea filtros si hace falta
            generoSeleccionado = null
            armarGenerosMenu()
            aplicarFiltros()
            binding.rvSeries.smoothScrollToPosition(0)
        }
        ...
        binding.navSeries.setOnClickListener {
            // Series = historial / continuar leyendo por ahora
    ...
    private fun crearOpcionMenu(texto: String, activo: Boolean, alTocar: () -> Unit): View {
        ...
        tv.setTextColor(resources.getColor(if (activo) R.color.accent2 else R.color.white, theme))
