    ...
    private fun revisarPopupRemoto() {
        lifecycleScope.launch {
            try {
                ...
                val dialogo = androidx.appcompat.app.AlertDialog.Builder(this@MainActivity)
                    .setTitle(popup.titulo)
                    .setMessage(popup.mensaje)
                    .setNegativeButton("Cerrar", null)

                if (!popup.texto_boton.isNullOrBlank() && !popup.url_boton.isNullOrBlank()) {
                    dialogo.setPositiveButton(popup.texto_boton) { _, _ ->
                        try {
                            startActivity(
                                android.content.Intent(
                                    android.content.Intent.ACTION_VIEW,
                                    android.net.Uri.parse(popup.url_boton)
                                )
                            )
                        } catch (e: Exception) {
                            android.widget.Toast.makeText(this@MainActivity, "No se pudo abrir el enlace.", android.widget.Toast.LENGTH_SHORT).show()
                        }
                    }
                }
